...
const { incrementerUtilisation } = require('./codePromoController');
...
exports.createReservation = async (req, res) => {
  try {
    const {
      ...
      paymentOption,
      nightsToPay,
      codePromo: codePromoString  // ✅ NOUVEAU
    } = req.body;
    ...
    let codePromoDoc = null;
    if (codePromoString) {
      codePromoDoc = await CodePromo.findOne({ code: codePromoString.toUpperCase() });
    }
    ...
    const reservationData = {
      ...
      source: 'website',
      // ✅ NOUVEAU : lier le code promo
      codePromo: codePromoDoc ? codePromoDoc._id : null,
      paiement: {
        amount: totalAmount,
        currency: 'XAF',
    ...
    // ✅ NOUVEAU : incrémenter le compteur du code promo
    if (codePromoDoc) {
      await incrementerUtilisation(codePromoDoc._id);
      console.log(`🎟️ Code promo ${codePromoDoc.code} incrémenté — réservation ${reservation._id}`);
    }
...
// 🔹 CRÉER UNE RÉSERVATION PUBLIQUE avec Secure Acceptance
exports.createReservationPublic = async (req, res) => {
  try {
    ...
    console.log('🔹 Création réservation publique:', {
      ...
      paymentOption,
      nightsToPay,
      codePromo: codePromoString || 'AUCUN'
    });
    ...
    if (codePromoString) {
      codePromoDoc = await CodePromo.findOne({ code: codePromoString.toUpperCase() });
      if (codePromoDoc) {
        console.log(`🎟️ Code promo trouvé: ${codePromoDoc.code} (titulaire: ${codePromoDoc.titulaire || 'hôtel'})`);
      }
    }
    ...
    switch (paymentOption) {
      case 'first-night':
        calculatedTotalAmount = chambre.price;
        calculatedNightsToPay = 1;
        break;
      case 'partial': {
      ...
      case 'full':
      default:
        calculatedTotalAmount = chambre.price * calculatedNights;
    ...
    if (!CybersourceSecure.isConfigured()) {
      ...
      return res.status(201).json({
        ...
        reservation,
        payment: { mockMode: true, ...mockParams }
      });
    }
    ...
    res.status(201).json({
      ...
      message: 'Réservation créée avec succès. Redirection vers le paiement.',
      reservation,
      payment: { form_action: paymentUrl, form_data: paymentParams }
    });
...
exports.paymentCallback = async (req, res) => {
  try {
    ...
    if (decision === 'ACCEPT') {
      ...
      if (reservation.paymentOption !== 'full') reservationStatus = 'partially_paid';
  ...
  try {
    ...
    if (req.user.role === 'admin') {
      reservations = await Reservation.find()
        ...
        .populate('codePromo', 'code type value');  // ✅ NOUVEAU
    } else {
...
exports.getReservationById = async (req, res) => {
  try {
    const reservation = await Reservation.findById(req.params.id)
      ...
      .populate('codePromo', 'code type value description');  // ✅ NOUVEAU
...
exports.updateReservation = async (req, res) => {
  try {
    ...
    ).populate('client chambre codePromo');  // ✅ NOUVEAU
...
exports.cancelReservation = async (req, res) => {
  try {
    ...
    if (reservation.paiement) reservation.paiement.status = 'refunded';
...
exports.confirmReservation = async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Réservation confirmée avec succès', reservation });
